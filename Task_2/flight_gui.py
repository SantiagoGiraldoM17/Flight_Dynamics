import numpy as np
from PyQt5.QtWidgets import QWidget, QHBoxLayout, QVBoxLayout, QGridLayout, QLabel, QTabWidget, QSplitter
...
class FlightDataViewer(QWidget):
    ...
    def _setup_aircraft_gl(self):
        ...
        # NED reference axes: (GL x=N, GL y=E, GL z=-D)
        self.gl_aircraft.addItem(_gl_axis([2.5,  0,   0],  (1, 0, 0, 1)))   # N red
        self.gl_aircraft.addItem(_gl_axis([0,  2.5,   0],  (0, 1, 0, 1)))   # E green
        self.gl_aircraft.addItem(_gl_axis([0,    0, -2.5], (0, 0, 1, 1)))   # D blue

        # Axis labels
        for pos, text, color in [
            ((2.8,  0,   0),  'N', (255, 80,  80,  255)),
            ((0,  2.8,   0),  'E', (80,  255, 80,  255)),
    ...
    def _build_vned_tab(self):
        tab = QWidget()
        tab.setStyleSheet("background: #1a1a1a;")
        self.tabs.addTab(tab, "  V_NED  ")
        layout = QVBoxLayout(tab)
        layout.setSpacing(8)
        layout.setContentsMargins(4, 4, 4, 4)

        t_max = self.history['times'][-1] if self.history is not None else 60.0
        labels = ['V_N — North (m/s)', 'V_E — East (m/s)', 'V_D — Down (m/s)']
        ...
        for j, (lbl, col) in enumerate(zip(labels, colors)):
            pw = self._make_plot_widget(lbl, col, t_max,
                                         show_xaxis=(j == 2))
            self.vned_plots.append(pw)
            self.vned_curves.append(pw.plot(pen=pg.mkPen(color=col, width=1)))
            layout.addWidget(pw)

        self._link_x_axes(self.vned_plots)
    ...
    def _build_trajectory_tab(self):
        ...
        pw2d.setLabel('left',   'Altitude (m)', color='white')
        pw2d.setTitle("2D Trajectory  (East vs Altitude)", color='white', size='10pt')
    ...
    def _refresh_text(self):
        ...
        text = (
            ...
            f"QUATERNION:\n"
            f"  [{self.quaternion[0]:+.4f},  {self.quaternion[1]:+.4f},\n"
            f"   {self.quaternion[2]:+.4f},  {self.quaternion[3]:+.4f}]\n"
            f"  θ_rot = {self.rot_angle:.2f}°\n"
            f"  ê = [{self.euler_axis[0]:+.3f},"
            f" {self.euler_axis[1]:+.3f},"
            f" {self.euler_axis[2]:+.3f}]\n\n"
            f"AERO ANGLES:\n"
    ...
    def _update_3d_aircraft(self):
        ...
        def gl_pt(i):
            return [pts_ned[0, i], pts_ned[1, i], -pts_ned[2, i]]
        ...
        for name, corners_body in _AC_PANELS.items():
            ...
            pts = (self.C_b_n @ corners_body.T).T   # (4, 3) NED
            verts = np.empty_like(pts)
            verts[:, 0] = pts[:, 0]   # N → GL x
            verts[:, 1] = pts[:, 1]   # E → GL y
        ...
        if v_norm > 1e-3:
            ...
            self.ac_velocity.setData(
                pos=np.array([[0, 0, 0], [vn, ve, -vd]], dtype=np.float32))
    ...
    def _update_timeseries(self):
        ...
        for j, curve in enumerate(self.vned_curves):
            curve.setData(x=t, y=h['vel_ned'][:i, j])

        pos_e = h['position'][:i, 1]
        pos_n = h['position'][:i, 0]
        alt   = h['altitude'][:i]

        self.traj_curve_2d.setData(x=pos_e, y=alt)
        self.traj_start_2d.setData(  x=[pos_e[0]],  y=[alt[0]])
        self.traj_current_2d.setData(x=[pos_e[-1]],  y=[alt[-1]])
